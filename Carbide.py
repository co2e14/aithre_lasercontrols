...
response = requests.put(laserEndPoint + "/v1/Basic/SelectedPresetIndex", data = json.dumps(1), headers = requestHeaders )
if response.status_code == 200:
    # Operation succeeded.
...
response = requests.post(laserEndPoint + "/v1/Basic/ApplySelectedPreset", headers = requestHeaders)
if response.status_code == 200:
    # Operation succeeded.
...
# Enable output.
response = requests.post(laserEndPoint + "/v1/Basic/EnableOutput", headers = requestHeaders)
...
# Close output.
response = requests.post(laserEndPoint + "/v1/Basic/CloseOutput", headers = requestHeaders)
print("Output is closed. Laser is not working.")

# When job is done leave laser at standing by state.
response = requests.post(laserEndPoint + "/v1/Basic/GoToStandby", headers = requestHeaders)
