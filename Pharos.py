...
laserEndPoint = "http://127.0.0.1:20022"

# Reading current preset index.
response = requests.get(laserEndPoint + "/v0/Basic/SelectedPresetIndex")
...
response = requests.put(
    laserEndPoint + "/v0/Basic/SelectedPresetIndex",
    data=json.dumps(1),
...
response = requests.post(
    laserEndPoint + "/v0/Basic/ApplySelectedPreset", headers=requestHeaders
)
if response.status_code == 200:
    # Operation succeeded.
...
response = requests.get(laserEndPoint + "/v0/Basic/ActualStateName")
if response.status_code == 200:
...
# Enable output.
response = requests.post(
    laserEndPoint + "/v0/Basic/EnableOutput", headers=requestHeaders
)
...
# Close output.
response = requests.post(
    laserEndPoint + "/v0/Basic/CloseOutput", headers=requestHeaders
)
print("Output is closed. Laser is not working.")

# When job is done leave laser at standing by state.
response = requests.post(
    laserEndPoint + "/v0/Basic/GoToStandby", headers=requestHeaders
)
